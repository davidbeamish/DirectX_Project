...
bool Sound::Initialize(HWND hwnd)
{
	...
	result = LoadWaveFile("../Engine/data/atmosphere.wav", &m_secondaryBuffer1);
	if(!result)
	{
		return false;
	}
...
bool Sound::InitializeDirectSound(HWND hwnd)
{
	...
	// In this case it is a .WAV file recorded at 44,100 samples per second in 16-bit stereo, which is an uncompressed cd audio format.
