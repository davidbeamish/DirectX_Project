...
bool OrthoWindow::InitializeBuffers(ID3D11Device* device, int windowWidth, int windowHeight)
{
	...
	left = (float)((windowWidth / 2) * -1) + m_position.x;

	// Calculate the screen coordinates of the right side of the window.
	right = left + (float)windowWidth;

	// Calculate the screen coordinates of the top of the window.
	top = (float)(windowHeight / 2) - m_position.y;
...
void OrthoWindow::SetPosition(D3DXVECTOR2 position)
{
	m_position = position;
}
D3DXVECTOR2 OrthoWindow::GetPosition() const
{
	return m_position;
}