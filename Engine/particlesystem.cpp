...
bool ParticleSystem::Initialize(ID3D11Device* device, WCHAR* textureFilename, D3DXVECTOR3 position, D3DXVECTOR3 positionDeviation, float positionRadius,  D3DXVECTOR3 velocity, D3DXVECTOR3 velocityDeviation, D3DXVECTOR3 acceleration, D3DXVECTOR4 color, D3DXVECTOR4 colorVelocity, D3DXVECTOR4 colorAcceleration, D3DXVECTOR2 size, D3DXVECTOR2 maxSize, D3DXVECTOR2 sizeVelocity, D3DXVECTOR2 sizeAcceleration, float particlesPerSecond, int maxParticleCount, float lifeTime, float lifeSpeed, float lifeAcceleration) 
{
	...
	//Set the life speed and maximum life time of particles.
	m_lifeSpeed = lifeSpeed;
	m_lifeTime = lifeTime;
	m_lifeAcceleration = lifeAcceleration;
...
void ParticleSystem::EmitParticles(float time)
{
	...
	if((emitParticle == true) && (m_currentParticleCount < (m_maxParticleCount - 1)))
	{
		...
		while(i != index)
		{
			...
			m_particleList[i].lifeSpeed			= m_particleList[j].lifeSpeed;
			m_particleList[i].lifeAcceleration	= m_particleList[j].lifeAcceleration;
		...
		m_particleList[index].lifeSpeed			= m_lifeSpeed;
		m_particleList[index].lifeAcceleration	= m_lifeAcceleration;
...
void ParticleSystem::UpdateParticles(float time)
{
	int i;
	...
	for(i=0; i<m_currentParticleCount; i++)
	{
		...
		m_particleList[i].lifeTime = m_particleList[i].lifeTime - (m_lifeSpeed * 0.016666f);
...
void ParticleSystem::KillParticles(float time)
{
	int i, j;
	...
	{
		if( (m_particleList[i].active == true) && ( (m_particleList[i].lifeTime < 0.0f) || (m_particleList[i].color.w < 0.0f) ))
		{
			...
			for(j=i; j<m_maxParticleCount-1; j++)
			{
				...
				m_particleList[j].lifeSpeed			= m_particleList[j+1].lifeSpeed;
				m_particleList[j].lifeAcceleration	= m_particleList[j+1].lifeAcceleration;
...
void ParticleSystem::SetInitializationVariables(D3DXVECTOR3 position, D3DXVECTOR3 positionDeviation, float positionRadius, D3DXVECTOR3 velocity, D3DXVECTOR3 velocityDeviation, D3DXVECTOR3 acceleration, D3DXVECTOR4 color, D3DXVECTOR4 colorVelocity, D3DXVECTOR4 colorAcceleration, D3DXVECTOR2 size, D3DXVECTOR2 maxSize, D3DXVECTOR2 sizeVelocity, D3DXVECTOR2 sizeAcceleration, float particlesPerSecond, int maxParticleCount, float lifeTime, float lifeSpeed, float lifeAcceleration)
{
	// Set initial position and random deviation of the position of the particles.
	m_position =  position;
	m_positionDeviation = positionDeviation;
...
int ParticleSystem::GetCurrentParticleCount() const
{
...
D3DXVECTOR3 ParticleSystem::GetPosition() const
{
	return m_position;
}

void ParticleSystem::SetVelocity(D3DXVECTOR3 velocity)
{
	m_velocity = velocity;
}

void ParticleSystem::SetAcceleration(D3DXVECTOR3 acceleration)
{
	m_acceleration = acceleration;
}

void ParticleSystem::SetColor(D3DXVECTOR4 color)
{
	m_color = color;
}

void ParticleSystem::SetColorVelocity(D3DXVECTOR4 colorVelocity)
{
	m_colorVelocity = colorVelocity;
}

void ParticleSystem::SetColorAcceleration(D3DXVECTOR4 colorAcceleration)
{
	m_colorAcceleration = colorAcceleration;
}
...
void ParticleSystem::SetSize(D3DXVECTOR2 size)
{
	m_size = size;
}

void ParticleSystem::SetMaxSize(D3DXVECTOR2 maxSize)
{
	m_maxSize = maxSize;
}


void ParticleSystem::SetSizeVelocity(D3DXVECTOR2 sizeVelocity)
{
	m_sizeVelocity = sizeVelocity;
}

void ParticleSystem::SetSizeAcceleration(D3DXVECTOR2 sizeAcceleration)
{
	m_sizeAcceleration = sizeAcceleration;
}
...
void ParticleSystem::SetEmissionState(const bool emissionState)
{
	m_emissionState = emissionState;
}

bool ParticleSystem::GetEmissionState() const
{
