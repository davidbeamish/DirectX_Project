...
CloudsShader::CloudsShader(const CloudsShader& other)
{
}
...
bool CloudsShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;


	// Initialize the vertex and pixel shaders.
	result = InitializeShader(device, hwnd, L"../Engine/clouds.vs", L"../Engine/clouds.ps");
	if(!result)
	{
		return false;
	}
...
bool CloudsShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, 
				 ...
				 D3DXVECTOR2 translation, D3DXVECTOR2 scale, D3DXVECTOR2 brightness)
{
	...
	result = SetShaderParameters(deviceContext, indexCount, worldMatrix, viewMatrix, projectionMatrix, cloudTexture1, cloudTexture2, perturbTexture, translation, scale, brightness);
	if(!result)
	{
		return false;
	}
...
bool CloudsShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
	...
	result = D3DX11CompileFromFile(vsFilename, NULL, NULL, "CloudsVertexShader", "vs_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, 
	...
	result = D3DX11CompileFromFile(psFilename, NULL, NULL, "CloudsPixelShader", "ps_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, 
...
void CloudsShader::ShutdownShader()
{
...
void CloudsShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	char* compileErrors;
...
void CloudsShader::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
