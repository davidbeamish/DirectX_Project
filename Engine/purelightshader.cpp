...
PureLightShader::PureLightShader(const PureLightShader& other)
{
}
...
bool PureLightShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;


	// Initialize the vertex and pixel shaders.
	result = InitializeShader(device, hwnd, L"../Engine/purelight.vs", L"../Engine/purelight.ps");
	if(!result)
	{
		return false;
	}
...
bool PureLightShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, 
							  D3DXMATRIX projectionMatrix , D3DXVECTOR3 lightDirection, D3DXVECTOR4 ambientColor, D3DXVECTOR4 diffuseColor, D3DXVECTOR3 cameraPosition, D3DXVECTOR4 specularColor, float specularPower, float deltavalue)
{
	...
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, lightDirection, ambientColor, diffuseColor, cameraPosition, specularColor, specularPower, deltavalue);
	if(!result)
	{
		return false;
	}
...
bool PureLightShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
	...
	// Create the variable constant buffer pointer so we can access the vertex shader constant buffer from within this .
	...
	// Create the camera constant buffer pointer so we can access the vertex shader constant buffer from within this .
...
void PureLightShader::ShutdownShader()
{
...
void PureLightShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	char* compileErrors;
...
void PureLightShader::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
